...
import { listColor, deleteColor } from "../../query/product/color";
...
const Details = () => {
  const [colorlist, setcolorlist] = useState(arrcolorProps);
  ...
  const [deletedata, setdeletedata] = useState<deleteProps>({
    ...
    type: "alert",
    successButtonText: "Delete color",
    successAction: () => {},
    cancelAction: () => {},
  });
  const [editdata, seteditdata] = useState<colorProps>({
    _id: "",
    name: "",
    code: "",
  });
  useEffect(() => {
    listColor().then((response) => {
  ...
  const successAction = () => {
    deleteColor(deleteid).then((response) => {
      setcolorlist(response.data.data);
      setdeleteaction(false);
    });
  };
  ...
  const editcolor = (data: colorProps) => {
    seteditdata(data);
    setshowaddedit(true);
  };
  return (
