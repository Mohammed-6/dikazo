...
import { listAttribute, deleteAttribute } from "../../query/product/attribute";
...
const Details = () => {
  const [attributelist, setattributelist] = useState(arrattributeProps);
  ...
  const [deletedata, setdeletedata] = useState<deleteProps>({
    ...
    type: "alert",
    successButtonText: "Delete attribute",
    successAction: () => {},
    cancelAction: () => {},
  });
  const [editdata, seteditdata] = useState<attributeProps>({
    _id: "",
    name: "",
    attributeValue: [],
  });
  ...
  const successAction = () => {
    deleteAttribute(deleteid).then((response) => {
      setattributelist(response.data.data);
      setdeleteaction(false);
    });
  };
  ...
  const editattribute = (data: attributeProps) => {
    seteditdata(data);
    setshowaddedit(true);
  };
  return (
      ...
      <div className="card">
        <div className="card-header py-2">
          <div className="flex items-center justify-between">
            <div className="">Attribute List</div>
            <div className="">
              <div className="flex gap-x-2">
                <div className="">
                  <button onClick={() => setshowaddedit(true)}>
                    <button className="btn btn-primary">Add attribute</button>
                  </button>
                </div>
              </div>
            </div>
